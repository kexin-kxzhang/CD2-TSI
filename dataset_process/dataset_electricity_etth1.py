...
class ETTh1_Dataset(Dataset):
    def __init__(self, seed=1, eval_length=48, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        df = pd.read_csv('./data/electricity/ETTh1.csv', index_col=0)       
        ...
        if missing_pattern == 'block':
            eval_mask = sample_mask(shape=(17420, 7), p=0.0015, p_noise=0.05, min_seq=12, max_seq=12 * 4, rng=self.rng)
            print(eval_mask[:5])
        elif missing_pattern == 'point':
            eval_mask = sample_mask(shape=(17420, 7), p=0., p_noise=0.25, max_seq=12, min_seq=12 * 4, rng=self.rng)
        gt_mask = (1-(eval_mask | (1-ob_mask))).astype('uint8')
        print(gt_mask[:5])
        print("gt_mask True:", np.sum(gt_mask))

        self.train_mean = np.zeros(7)
        self.train_std = np.zeros(7)
        for k in range(7):
            tmp_data = df.iloc[:, k][ob_mask[:, k] == 1] 
            self.train_mean[k] = tmp_data.mean()
            self.train_std[k] = tmp_data.std()

        path = "./data/electricity/ETTh1_meanstd.pk"
...
def get_dataloader_etth1(batch_size, device, seed=1, val_len=0.1, num_workers=4, 
                   missing_pattern='block', target_strategy='hybrid', missing_ratio=0.1):
    dataset = ETTh1_Dataset(seed=seed, mode="train", missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True
    )
    dataset_test = ETTh1_Dataset(seed=seed, mode="test", missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False
    )
    dataset_valid = ETTh1_Dataset(seed=seed, mode="valid", val_len=val_len, missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
