...
class ETTh2_Dataset(Dataset):
    def __init__(self, seed=1, eval_length=48, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        df = pd.read_csv('./data/electricity/ETTh2.csv', index_col=0)       
        ...
        path = "./data/electricity/ETTh2_meanstd.pk"
...
def get_dataloader_etth2(batch_size, device, seed=1, val_len=0.1, num_workers=4, 
                   missing_pattern='block', target_strategy='hybrid', missing_ratio=0.1):
    dataset = ETTh2_Dataset(seed=seed, mode="train", missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True
    )
    dataset_test = ETTh2_Dataset(seed=seed, mode="test", missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False
    )
    dataset_valid = ETTh2_Dataset(seed=seed, mode="valid", val_len=val_len, missing_pattern=missing_pattern,
                             target_strategy=target_strategy)
